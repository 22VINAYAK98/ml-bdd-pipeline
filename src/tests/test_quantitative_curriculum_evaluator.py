...
from src.evaluation.metrics.quantitative_evaluator import QuantitativeEvaluator
...
def main():
    ...
    evaluator = QuantitativeEvaluator(
        ...
        output_dir=("outputs/" "evaluation/" "metrics"),
    )

    easy_results = evaluator.evaluate(
        records=(benchmark.get_easy_records())[:50],
        scenario_name="easy",
    )

    medium_results = evaluator.evaluate(
        records=(benchmark.get_medium_records())[:50],
        scenario_name="medium",
    )

    hard_results = evaluator.evaluate(
        records=(benchmark.get_hard_records())[:50],
        scenario_name="hard",
    )
