...
class DensityAnalyzer:
    ...
    def run(self):
        ...
        for record in self.records:
            ...
            object_counts.append(len(record.annotations))
...
class OcclusionAnalyzer:
    ...
    def analyze_occlusion(self):
        ...
        return {
            ...
            "occlusion_ratio": occluded_objects / total_objects,
        }
    ...
    def analyze_truncation(self):
        ...
        return {
            ...
            "truncation_ratio": truncated_objects / total_objects,
        }

    def run(self):

        return {
            "occlusion_statistics": self.analyze_occlusion(),
            "truncation_statistics": self.analyze_truncation(),
        }
    ...
    def run(self):
        ...
        for record in self.records:
            ...
            for annotation in record.annotations:
                ...
                distribution[timeofday][annotation.category] += 1

        final_distribution = {}

        for timeofday, counter in distribution.items():

            final_distribution[timeofday] = dict(counter)
...
class VulnerableRoadUserAnalyzer:
    ...
    def run(self):
        ...
        for record in self.records:
            ...
            is_night = record.timeofday == "night"

            for annotation in record.annotations:

                is_vru = annotation.category in VRU_CLASSES

                is_occluded = annotation.occluded

                if is_night:

                    if is_vru and is_occluded:

                        stats["night_occluded_vru_objects"] += 1

                    elif not is_vru:

                        stats["night_other_objects"] += 1

                else:

                    if is_vru and is_occluded:

                        stats["day_occluded_vru_objects"] += 1

                    elif not is_vru:

                        stats["day_other_objects"] += 1
...
class ObjectAnalysisPipeline:
    ...
    def run(self):
        ...
        return {
            "class_distribution": ClassDistributionAnalyzer(self.records).run(),
            "density_statistics": DensityAnalyzer(self.records).run(),
            "occlusion_statistics": OcclusionAnalyzer(self.records).run(),
            "bounding_box_statistics": BoundingBoxAnalyzer(self.records).run(),
            "timeofday_class_distribution": ClassDistributionByTimeOfDay(
                self.records
            ).run(),
            "vru_analysis": VulnerableRoadUserAnalyzer(self.records).run(),
        }
